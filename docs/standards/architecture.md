...
- **Infrastructure**: Firestore へのアクセスは `FirestoreTemplateRepository` と `FirestoreUnitOfWork` が担い、必須コレクションの整備、既定テンプレートの注入、埋め込み/抽選モード設定の保持を担当します。`services.app_context` が `create_template_repository()` を通じてシングルトンの Repository を払い出し、プレゼンテーション層から直接利用します。【src/infrastructure/firestore/template_repository.py†L150-L450】【src/infrastructure/firestore/unit_of_work.py†L24-L122】【src/services/app_context.py†L1-L46】
...
4. `create_template_repository()` は環境変数/URL/ローカルファイルから Firebase 認証情報を解決し、FirestoreTemplateRepository のシングルトンを初期化します。URL 参照時はキャッシュを用いて I/O を抑制します。【src/services/app_context.py†L1-L46】【src/infrastructure/config/firebase_credentials.py†L1-L109】
...
- 旧 `src/views/` / `src/bot/` の再エクスポートは廃止済みです。ビューやクライアントは `presentation.discord` の正規モジュールを直接参照し、新たな互換レイヤーを追加しない方針とします。
- レガシー呼び出しを置き換える際は、本書と `module-overview` の両方を更新し、互換 API の追加ではなく正規モジュールへの移行を優先してください。【docs/references/module-overview.md†L1-L80】
